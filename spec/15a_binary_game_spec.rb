...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        incorrect_input = 'x'
        valid_input = '3'
        allow(game_input).to receive(:gets).and_return(incorrect_input, valid_input)
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        another_one = 'q'
        valid_input = '3'
        allow(game_input).to receive(:gets).and_return(incorrect_input, another_one, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    subject(:verify) {described_class.new(1, 20)}
    # Note: #verify_input will only return a number if it is between?(min, max)

    context 'when given a valid input as argument' do
      it 'returns valid input' do
        min = verify.instance_variable_get(:@minimum)
        max = verify.instance_variable_get(:@maximum)
        choice = 12
        verify_input = verify.verify_input(min, max, choice)
        expect(verify_input).to eq(12)
      end
    end
  ...
  describe '#display_turn_order' do
    ...
    subject(:new_subject) {described_class.new(1, 20)}
    ...
    before do
      ...
      allow(new_subject).to receive(:display_guess)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect{ new_subject.display_turn_order(binary_search_turn) }.to change { new_subject.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_search_turn).to receive(:make_guess).once
      subject.display_turn_order(binary_search_turn)
    end
