...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:coffee_lavazza) {described_class.new("coffee")} 
      # remove the 'x' before running this test
      it 'is coffee' do
        expect(coffee_lavazza.type).to eq("coffee")
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(coffee_lavazza.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:coffee_lavazza) {described_class.new("coffee", 17)}
      # remove the 'x' before running this test
      it 'is full' do
        expect(coffee_lavazza.full?).to be true
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:coffee_lavazza) {described_class.new("coffee", 12)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(coffee_lavazza.full?).not_to be true
      end
    end
  end
end
